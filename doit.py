#!/usr/pkg/bin/python
#
# This is Anita, the Automated NetBSD Installation Test Application
#
...
class Version:
	...
	for floppy in ['boot-com1.fs', 'boot2.fs']:
	    ftp_if_missing(self.dist_url(), self.ftp_local_dir(), "i386/installation/floppy/" + floppy)
	# "xbase", "xcomp", "xetc", "xfont", "xserver", 
	for set in [ "base", "comp", "etc", "games", "man", "misc", "text", "kern-GENERIC" ]:
	    ftp_if_missing(self.dist_url(), self.ftp_local_dir(), "i386/binary/sets/" + set + ".tgz")
	...
	if os.path.isfile(self.iso_path()):
            return
	spawn("makefs", ["makefs", "-t", "cd9660", "-o", "rockridge", self.iso_path(), self.ftp_local_dir()])
	...
	spawn("qemu-img", ["qemu-img", "create", self.wd0_path(), "1500M"])
	child = pexpect.spawn("qemu", ["qemu", "-m", "32", "-hda", self.wd0_path(), \
...
class DailyBuild(Version):
	...
	dash_ver = re.sub("[\\._]", "-", self.ver)
        return "http://ftp.netbsd.org/pub/NetBSD-daily/netbsd-" + dash_ver + "/" + self.timestamp + "/"
